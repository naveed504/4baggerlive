...
var drop = function (info) {
    var o = {
        ...
        open: false,
        html: {
            select: ww.get(info.selector)[0],
            options: ww.get(info.selector + " option"),
            parent: undefined,
        },
        init: function () {
            //Setup Drop HTML
            this.html.parent = ww.get(info.selector)[0].parentNode;
            this.html.drop = ww.template('<div class="drop"></div>');
            this.html.dropDisplay = ww.template(
                '<div class="drop-display">Display</div>'
            );
            this.html.dropOptions = ww.template(
                '<div class="drop-options">Options</div>'
            );
            this.html.dropScreen = ww.template(
                '<div class="drop-screen"></div>'
            );
        ...
        renderDrop: function () {
            var that = this;
            var parentHTML = ww.template("<div></div>");
            this.selected.forEach(function (select, index) {
                var option = that.options[select.index];
                var childHTML = ww.template(
                    '<span class="item ' +
                        select.state +
                        '">' +
                        option.html +
                ...
                var childCloseHTML = ww.template(
        ...
        isSelected: function (index) {
            var check = false;
            this.selected.forEach(function (select) {
                if (select.index == index && select.removed == false)
                    check = true;
            });
            return check;
        },
...
if (typeof bats != "undefined") {
    var bat = JSON.parse(bats);
    const allBats = ["Wood", "BBCOR", "-5", "-8", "-10"];
    let array = [];
    if(bat != null || bat !=undefined) {      //Naveed
    for (let i = 0; i < bat.length; i++) {        
        array.push(allBats.indexOf(bat[i]));        
    }
}
...
let i = 0;
$("#addEventVenu").on("click", function () {
    ++i;
    let addVenu =
        '<label class="label__wrapper required">Event Venue</label>' +
        '<input type = "text" name = "event_venue" class = "form-control input__box--wrapper">';
    $("#addEventVenue").prepend(addVenu);
});
